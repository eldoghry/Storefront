...
export default (req: Request, res: Response, next: NextFunction) => {
  ...
  const category_id: number | undefined = req.body.category_id ? parseInt(req.body.category_id) : undefined;
  ...
  if (invalidMsg.length) return customErrorRes(res, 400, `${invalidMsg.join(', ')} are required to update product`);
  else next();
};
